...
sub do_testHtmlOutput {
    ...
    if ($doRender) {
        ...
        $expected =
          Foswiki::Func::expandCommonVariables( $expected, $webName, $topicName );
        $expected =
          Foswiki::Func::renderText( $expected, $webName, $topicName );
    }
...
sub doTest {
    ...
    my $result =
      Foswiki::Func::expandCommonVariables( $raw_text, $this->{test_topic},
        $this->{test_web} );
